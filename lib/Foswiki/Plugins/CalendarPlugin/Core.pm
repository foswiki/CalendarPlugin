...
MESSAGE
        ...
        || ( ( $y <= $lastYear ) && ( $m <= $lastMonth ) ) )
    {
        ...
        # _fetchDays, the matched lines are deleted from the bullet
        ...
        foreach $d (@days) {
            ...
            eval {
                ...
                my $date1 = Date::Calc::Date_to_Days( $y, $months{$mm1}, $dd1 );
                my $date2 = Date::Calc::Date_to_Days( $y, $months{$mm2}, $dd2 );
        ...
        foreach $d (@days) {
            my ( $dd, $mm, $yy, $xs, $xcstr, $descr ) = split( /\|/, $d );
            eval {
                if ( $yy == $y && $months{$mm} == $m )
                {
        ...
        foreach $d (@days) {
            my ( $dd, $mm, $xs, $xcstr, $descr ) = split( /\|/, $d );
            eval {
                my @xmap;
                if ( length($xcstr) > 9 ) {
                    @xmap = _fetchxmap( $xcstr, $y, $m );
        ...
        foreach $d (@days) {
            my ( $nn, $dd, $xs, $xcstr, $descr ) = split( /\|/, $d );
            eval {
                my @xmap;
                if ( length($xcstr) > 9 ) {
                    @xmap = _fetchxmap( $xcstr, $y, $m );
                ...
                if ( $hd <= Date::Calc::Days_in_Month( $y, $m ) && $xmap[$hd] )
                {
        ...
        foreach $d (@days) {
            ...
            eval {
                ...
                my ( $ny, $nm );
                my $hd =
                  Date::Calc::Nth_Weekday_of_Month_Year( $y, $m, $wdays{$dd},
                    1 );
                do {
        ...
        foreach $d (@days) {
            my ( $dd, $dy, $mn, $xs, $xcstr, $descr ) = split( /\|/, $d );
            eval {
                $mn = $months{$mn};
                my @xmap;
                if ( length($xcstr) > 9 ) {
        ...
        foreach $d (@days) {
            my ( $p, $dd, $mm, $yy, $xs, $xcstr, $descr ) = split( /\|/, $d );
            eval {
                my @xmap;
                if ( length($xcstr) > 9 ) {
                    @xmap = _fetchxmap( $xcstr, $y, $m );
